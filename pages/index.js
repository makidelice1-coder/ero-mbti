...
const QUESTIONS = [
  ...
  },
  {
    ...
    choices: [
      ...
      { label: 'かなり勇気がいる。相手から言われたらギリ送れるかも', score: { I: 1 } },
  ...
  if (phase === 'top') return (
    <>
      <Head>
        ...
        <meta property="og:description" content="全10問・約2分。あなたの性的傾向を16タイプで丸裸にします。" />
      ...
      <div style={{...base, minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem 1.5rem', textAlign:'center'}}>
        ...
        <p style={{fontSize:11, color:'#555', marginTop:'1.25rem', letterSpacing:'0.1em'}}>全10問 / 約2分</p>
      </div>
    </>
  )
  ...
  return (
    <>
      ...
      <div style={{...base, minHeight:'100vh', padding:'2.5rem 1.5rem', maxWidth:600, margin:'0 auto'}}>
        ...
        <div style={{fontSize:19, fontWeight:300, lineHeight:1.6, marginBottom:'1.75rem'}}>{q.text}</div>
        {q.choices.map((c, i) => (
          <button key={i} onClick={() => answer(c.score)}
            ...
            style={{
              ...
              padding:'15px 20px', borderRadius:10, fontSize:14,
              textAlign:'left', cursor:'pointer', marginBottom:10,
              lineHeight:1.5, transition:'all .15s'
            }}
          >
            <span style={{color: hov===i ? '#e06080' : '#555', marginRight:10, fontSize:12}}>
              {['A','B','C','D'][i]}
            </span>
            {c.label}
          </button>
